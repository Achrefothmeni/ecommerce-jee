	...
	public consulterCat() {
		...
		btnConsulter.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				List<Categorie> listeC = proxy.listeCategorie();
				for(int i = 0; i <listeC.size(); i++)
		        {
		            table.setModel(new DefaultTableModel(
		                    new Object[][] {
		                            {listeC.get(i).getIdCat(),listeC.get(i).getDesc(),listeC.get(i).getNomCat()}
		                    },
		                    new String[] {
		                        "Id Categorie", "Description", "Nom Categorie"
		                    }
		                ));
		        }
			}
